 ...
 * @return string
 */
function theme_mcc_get_extra_scss($theme) {
    $scss = $theme->settings->scss;

    $scss .= theme_mcc_set_headerimg($theme);

    $scss .= theme_mcc_set_topfooterimg($theme);

    $scss .= theme_mcc_set_loginbgimg($theme);
 ...
 * @return string
 */
function theme_mcc_set_headerimg($theme) {
    global $OUTPUT;
    ...
    $headercss = "#page-site-index.notloggedin #page-header {background: linear-gradient(rgba(36, 43, 87, 0.7), rgba(149, 224, 230, 0.7)),url('$headerimg');}";

    return $headercss; //linear-gradient(rgba(36, 43, 87, 0.8), rgba(149, 224, 230, 0.8)),
}
 ...
 * @return string
 */
function theme_mcc_set_topfooterimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_mcc_set_loginbgimg($theme) {
    global $OUTPUT;
    ...
    $headercss = "#page-login-index.mcc-login #page-wrapper #page {background-image: url('$loginbgimg');}";
 ...
 * @return string
 */
function theme_mcc_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_mcc', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_mcc and not theme_boost (see the line above).
    ...
    // mcc scss.
    $mccvariables = file_get_contents($CFG->dirroot . '/theme/mcc/scss/mcc/_variables.scss');
    $mcc = file_get_contents($CFG->dirroot . '/theme/mcc/scss/mcc.scss');
 ...
 * @return string
 */
function theme_mcc_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 * @return mixed
 */
function theme_mcc_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    $theme = theme_config::load('mcc');
 ...
 * @return string
 */
function theme_mcc_get_setting($setting, $format = false) {
    $theme = theme_config::load('mcc');
...
/**
 * Extend the mcc navigation
 *
 * @param flat_navigation $flatnav
 */
function theme_mcc_extend_flat_navigation(\flat_navigation $flatnav) {
    theme_mcc_rebuildcoursesections($flatnav);

    theme_mcc_delete_menuitems($flatnav);
}
 ...
 * @param flat_navigation $flatnav
 */
function theme_mcc_delete_menuitems(\flat_navigation $flatnav) {
 ...
 * @param flat_navigation $flatnav
 */
function theme_mcc_rebuildcoursesections(\flat_navigation $flatnav) {
    global $PAGE;
    ...
    if ($PAGE->course->format != 'singleactivity') {
        $coursesectionsoptions = [
            'text' => get_string('coursesections', 'theme_mcc'),
            'shorttext' => get_string('coursesections', 'theme_mcc'),
